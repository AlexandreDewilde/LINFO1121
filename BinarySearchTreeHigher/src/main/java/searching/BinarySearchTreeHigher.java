...
public class BinarySearchTreeHigher<Key extends Comparable<Key>, Value> {
    ...
    public Key min(Node current) {
        if (current.left != null) return min(current.left);
        return current.key;
    }
    ...
    public Key higher(Node current, Key key) {
        if (current == null) return null;
        if (current.key.compareTo(key) > 0) {
            Key res = higher(current.left, key);
            if (res != null && res.compareTo(current.key) < 0) {
                return res;
            }
            return current.key;
        }
        return higher(current.right, key);
    }
