...
public class MinMaxHeap<Key extends Comparable<Key>> {
    ...
    public Key max() {
        if (size == 1) return content[1];
         if (size == 2 || content[2].compareTo(content[3]) >= 0) {
             return content[2];
         }
         return content[3];
    }
    ...
    public void swim(int position) {
        ...
        if ((depth & 1) == 0) {
            if (content[position].compareTo(content[position/2]) > 0) {
    ...
    public void swimMin(int position) {
        if (position / 2/2 > 0 && content[position].compareTo(content[position/2/2]) < 0) {
            swap(position, position/2/2);
            swimMin(position/2/2);
        }
    }
