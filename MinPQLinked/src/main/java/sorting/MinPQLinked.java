...
public class MinPQLinked<Key> {
    ...
    public Key delMin() {
        ...
        root.size--;
        while (current.left != null || current.right != null) {
            if (current.right == null || !isPowerOfTwo(current.left.size+1) || (isPowerOfTwo(current.right.size+1) && current.left.size > current.right.size)) {
        ...
        else if (prev != null && prev.right == current) {
            prev.right = null;
        }
        else if(prev != null) {
            assert false;
        ...
        while (current.left != null || current.right != null) {
            Node small = current;
            if (greater(small, current.left)) {
                small = current.left;

            }
            if (current.right != null && greater(small, current.right)) {
                small = current.right;
