...
public class WordTransformationSP {
    ...
    public static int minimalCost(String from, String to) {
        ...
        while (q.size() > 0) {
            ...
            if (p.val > dst.get(s)) continue;
            for (int i = 0; i < from.length(); i++) {
                for (int j = i; j <= from.length(); j++) {
                    String string_to = rotation(s, i, j);
                    if (!dst.containsKey(string_to) || dst.get(string_to) > dst.get(s) + (j - i)) {
                        dst.put(string_to, dst.get(s) + (j - i));
                        q.add(new Pair(string_to, dst.get(string_to)));
                    }
                }
            }
        }
