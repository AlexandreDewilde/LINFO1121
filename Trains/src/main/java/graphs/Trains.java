...
public class Trains {
    ...
    public static Map<String, Integer> reachableEarliest(HashMap<StationTime, LinkedList<StationTime>> relations, StationTime startPoint) {
        ...
        dst.put(startPoint.station, startPoint.time);
        Queue<StationTime> pq = new PriorityQueue<>();
        pq.add(startPoint);
        while (pq.size() > 0) {
            StationTime station = pq.poll();
            for (int i = 0; i <1000; i++) {
                if (relations.get(new StationTime(station.station, station.time+i)) == null) continue;
                for (StationTime adj : relations.get(new StationTime(station.station, station.time+i))) {
                    if (!dst.containsKey(adj.station) || dst.get(adj.station) > adj.time) {
                        dst.put(adj.station, adj.time);
                        pq.add(adj);
                    }
                }
            }
