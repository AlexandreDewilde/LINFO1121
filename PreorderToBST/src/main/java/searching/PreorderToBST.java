...
public class PreorderToBST {
    ...
    */
    ...
    public PreorderToBST(int [] preOrderInput) {
        ...
        root = preorderRead(preOrderInput,Integer.MIN_VALUE,Integer.MAX_VALUE);
        current = 0;
    }

    public Node preorderRead(int [] preOrderInput, int min, int max) {
        if (current >= preOrderInput.length) {
            return null;
        }
        if (preOrderInput[current] > max || preOrderInput[current] < min) return null;
        int i = current;
        current++;
        Node left = preorderRead(preOrderInput, min, preOrderInput[i]);
        Node right = preorderRead(preOrderInput, preOrderInput[i], max);
